...
from statnight.parameters import Named, Range, Observable, Variable, Constant
...
def test_methods():
    ...
    assert minuit_const["fix_const"] is True
...
def test_repr():
    ...
    var = Variable("var", (1, 2))
    repr_var = "Variable('var', initvalue=1.5, range=(1, 2)"
    repr_var += ", initstep=0.01)"
    assert repr(var) == repr_var

    def f(a):
        return (a-2)**2
    var = Variable("var", (1, 2), initvalue=1.2, initstep=0.1, constraint=f)
    repr_var = "Variable('var', initvalue=1.2, range=(1, 2)"
    repr_var += ", initstep=0.1, constraint={0})".format(f)
    assert repr(var) == repr_var
